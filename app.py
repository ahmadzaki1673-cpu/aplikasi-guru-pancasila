...
# --- NAVIGASI UTAMA ---
st.title("📱 Sistem Guru & Wali Kelas")
menu = st.sidebar.radio("MENU UTAMA", ["📝 Jurnal & Mapel", "📊 Penilaian Siswa", "👨‍🏫 Wali Kelas 8"])
...
if menu == "📝 Jurnal & Mapel":
    ...
    with st.form("form_jurnal"):
        ...
        kls = c1.selectbox("Pilih Kelas", list(DAFTAR_SISWA.keys()))
        materi = c2.text_input("Materi Pembelajaran")
        
        st.write("---")
        st.write("**Presensi Siswa (Jam Pelajaran):**")
        status_mapel = {}
        for nama in DAFTAR_SISWA[kls]:
            col_n, col_o = st.columns([1, 2])
            col_n.write(f"{nama}")
            status_mapel[nama] = col_o.radio(f"S-{nama}", ["H", "S", "I", "A"], horizontal=True, key=f"m_{nama}", label_visibility="collapsed")
        
        if st.form_submit_button("Simpan Jurnal & Absen"):
            abs_str = ", ".join([f"{n}: {s}" for n, s in status_mapel.items()])
            simpan_data({"Tanggal": str(tgl), "Kelas": kls, "Materi": materi, "Presensi": abs_str}, FILE_JURNAL)
...
# --- 2. PENILAIAN SISWA ---
    ...
    with t1:
        with st.expander("Form Input Nilai Baru", expanded=True):
            cx, cy = st.columns(2)
            k_n = cx.selectbox("Kelas ", list(DAFTAR_SISWA.keys()))
